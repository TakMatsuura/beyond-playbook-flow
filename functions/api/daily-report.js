...
export async function onRequestGet(context) {
  ...
  // 集計
  const pv = parseInt((await env.FLOW_ANALYTICS.get(`pv:${targetDate}`)) || '0', 10);
  const uu = parseInt((await env.FLOW_ANALYTICS.get(`uucount:${targetDate}`)) || '0', 10);
  const submissions = parseInt((await env.FLOW_ANALYTICS.get(`submit:${targetDate}`)) || '0', 10);

  // パス別 PV (上位5)
  const pathList = await env.FLOW_ANALYTICS.list({ prefix: `path:${targetDate}:` });
  const pathStats = [];
