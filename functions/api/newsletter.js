...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (env.FLOW_ANALYTICS) {
      ...
      await env.FLOW_ANALYTICS.put(key, JSON.stringify({ email, source, at: jstTime }), {
        expirationTtl: 365 * 24 * 60 * 60,
      });
      // 日次カウント
      const ckey = `newsletter_count:${jstDate}`;
      const cur = parseInt((await env.FLOW_ANALYTICS.get(ckey)) || '0', 10);
      await env.FLOW_ANALYTICS.put(ckey, String(cur + 1), { expirationTtl: 90 * 24 * 60 * 60 });
    }
