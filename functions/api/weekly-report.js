...
async function aggregateWeek(env, dates) {
  ...
  for (const date of dates) {
    const p = parseInt((await env.FLOW_ANALYTICS.get(`pv:${date}`)) || '0', 10);
    const u = parseInt((await env.FLOW_ANALYTICS.get(`uucount:${date}`)) || '0', 10);
    const s = parseInt((await env.FLOW_ANALYTICS.get(`submit:${date}`)) || '0', 10);
    const n = parseInt((await env.FLOW_ANALYTICS.get(`newsletter_count:${date}`)) || '0', 10);
...
function formatReport(start, end, w, p) {
  ...
  return [
    ...
    `📊 https://flow.beyond-holdings.co.jp/admin/`,
  ].join('\n');
}
